...
with tab4:
    ...
    if st.button("Calculate Asthma Risk"):
        asthma_risk = calculate_asthma_risk(frequency_of_symptoms, nighttime_symptoms, inhaler_use, fev1)
...
with tab5:
    ...
    for condition, risk in results.items():
        ...
        if risk == "High":
            ...
            st.write("- **Content**: Assess maintenance of a healthy lifestyle, reinforce preventive practices.")
            
            # Outcome Evaluation Plan
            st.write("### Outcome Evaluation Plan:")
            st.write("- **Timeframe**: Annual review of health metrics to ensure stability in condition.")
            st.write("- **Adjustments**: Minimal if health metrics remain stable, further preventive counseling if necessary.")
